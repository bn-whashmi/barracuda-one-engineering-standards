...
def collect_checks(repo: str, revision: str, token: str, manifest: dict[str, Any], wait_seconds: int = 0) -> dict[str, Any]:
    ...
    for check in check_runs:
        name = check.get("name")
        current = by_name.get(name) if name else None
        check_key = (check.get("started_at") or "", check.get("id") or 0)
        current_key = (
            (current.get("started_at") or "", current.get("id") or 0)
            if current
            else ("", 0)
        )
        if name and (current is None or check_key > current_key):
            by_name[name] = check
